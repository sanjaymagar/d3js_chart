...
function drawSimpleBarChart(json, graphId) {
  ...
  var width = _width - margin.left - margin.right,
    ...
    yMaxValue = d3.max(values.map(d => d.y));
  ...
  var bar = svg.selectAll('.bar').append('g').data(values).enter().append('g');
  ...
  bar
    ...
    .on('mouseover', () => div.style('display', null))
    .on('mouseout', () => div.style('display', 'none'))
    .on('mousemove', d => {
      div
        .html('Age Gap: ' + d.x + '<br/>' + 'Member : ' + formatData(d.y))
